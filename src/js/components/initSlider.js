...
const startSlider = () => {
    ...
    const checkSlider = () => {
        if ((activeSlide + 1 === sliderItems.length && document.documentElement.offsetWidth > 560) || activeSlide === sliderItems.length) {
            btnNextSlide.style.display = 'none';
        } else {
            btnNextSlide.style.display = '';
        }

        if ((activeSlide === 1 && document.documentElement.offsetWidth < 560) || activeSlide === 0) {
    ...
    const nextSlide = () => {
        ...
        pos = -sliderItems[0].clientWidth * activeSlide;
    ...
    const prevSlide = () => {
        sliderItems[activeSlide]?.classList.remove('slider__item_active');
        pos = -sliderItems[0].clientWidth * (activeSlide - 2);
...
window.addEventListener('resize', () => {
    if (activeSlide + 1 > sliderItems.length && document.documentElement.offsetWidth > 560) {
        activeSlide = sliderItems.length - 1;
        sliderItems[activeSlide]?.classList.add('slider__item_active');
    }
    pos = -sliderItems[0].clientWidth * (activeSlide - 1);
    sliderList.style.transform = `translateX(${pos}px)`;
    checkSlider();
})
